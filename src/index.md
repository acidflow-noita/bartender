...
const parseURLParams = () => {
  ...
  const reagentsParam = urlParams.get("reagents");
  ...
  if (productParam) {
    product = productParam.trim();
  }

  return { reagents, product };
};

const updateURL = (reagents, product) => {
  const url = new URL(window.location);

  if (reagents.length > 0) {
    url.searchParams.set("reagents", reagents.join(","));
  } else {
    url.searchParams.delete("reagents");
  }
...
```js
{
  ...
  selectedReagents = urlParams.reagents;
  selectedProduct = urlParams.product;
  ...
  const initializeApp = async () => {
    try {
      ...
      const [
        ...
        waitForElement("shareButtonContainer"),
      ...
      function updateUI() {
        try {
          ...
          updateURL(selectedReagents, selectedProduct);
        } catch (error) {
      ...
      // Create share button
      const shareButton = Inputs.button("Share this recipe", {
        value: null,
        reduce: () => {
          ...
          if (navigator.share) {
            navigator
              .share({
                title: "Noita Reactions Recipe",
                text: "Check out this Noita reaction recipe!",
                url: currentURL,
              })
              .catch((err) => {
      ...
      if (selectedReagents.length > 0) {
        requestAnimationFrame(() => {
          selectedReagents.forEach((reagent) => {
            safeChoicesOperation(reagentChoices, () => reagentChoices.setChoiceByValue(reagent));
          });
        });
      }
      ...
      if (selectedProduct) {
        requestAnimationFrame(() => {
          safeChoicesOperation(productChoices, () => productChoices.setChoiceByValue(selectedProduct));
        });
      }
...
<div class="grid grid-cols-4">
    ...
    <div class="card grid-colspan-1" id="shareButtonContainer"></div>
</div>
