...
$urlsFile = Join-Path -Path $PSScriptRoot -ChildPath "spells_images_urls_list.txt"
$resultsFile = Join-Path -Path $PSScriptRoot -ChildPath "SPELLS_RAW_IMAGES_URLS_RESPONSES.txt"

# Read each URL from the spells_images_urls_list.txt file
...
foreach ($url in $urls) {
    ...
    try {
        ...
        # Append the JSON response to SPELLS_RAW_IMAGES_URLS_RESPONSES.txt with increased depth
