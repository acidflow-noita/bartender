...
import {predictionRows, sourceState, advanceSearch, searchProgress, compileGoals} from "../src/fungal/engine.js";
...
test("Air is the original goal sentinel, not a removed material or alternate transformation", () => {
  ...
  assert.equal(model.get().search.error, compileGoals(model.get().goals).error);
